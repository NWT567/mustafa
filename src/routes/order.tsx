...
function MenuCard({
  ...
  return (
    <article
      ...
      className="group flex min-h-[29.5rem] scroll-mt-32 flex-col overflow-hidden rounded-[1.6rem] border border-border/90 bg-card shadow-luxury transition duration-300 target:border-gold target:ring-2 target:ring-gold/35 target:shadow-glow hover:-translate-y-1 hover:border-gold/55 hover:shadow-glow"
    >
      <div className="relative aspect-[1.04/1] overflow-hidden bg-espresso">
        <img
          src={item.image}
          alt={item.name}
          loading="lazy"
        ...
        />
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-espresso/20 via-transparent to-black/10" />
        <span className="absolute right-3 top-3 max-w-[calc(100%-1.5rem)] truncate rounded-full border border-cream/65 bg-cream/95 px-3 py-1 text-[9px] font-medium uppercase tracking-[0.2em] text-gold-ink shadow-luxury backdrop-blur">
          {item.category}
        </span>
        {qty > 0 && (
          <span className="absolute left-3 top-3 rounded-full bg-espresso/95 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.16em] text-cream shadow-luxury backdrop-blur">
            {qty} in cart
          </span>
        )}
      </div>

      <div className="flex flex-1 flex-col px-5 pb-5 pt-5 sm:px-5">
        <div className="flex items-start justify-between gap-3">
          <h4 className="min-w-0 font-display text-[1.45rem] leading-[1.08] text-foreground">
            {item.name}
          </h4>
          <div className="shrink-0 text-right">
            <div className="text-[9px] font-medium uppercase tracking-[0.23em] text-gold-ink">
              Price
            </div>
            <div className="font-display text-[1.45rem] leading-none text-gold-gradient">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 pt-6">
          <button
            ...
            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-gradient-to-br from-gold-soft to-caramel px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-espresso shadow-glow transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2"
          >
            <Plus className="h-3.5 w-3.5" /> Add on
          </button>
          <div className="text-right text-[8px] font-medium uppercase tracking-[0.28em] text-muted-foreground">
            Ready ~ 6 min
          </div>
        </div>
      </div>
